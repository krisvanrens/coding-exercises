...
/// Token reader abstraction used in version 09.
...
#[derive(Debug, PartialEq)]
pub enum Token {
    /// Operand taking a signed 64-bit integer with a value.
    Operand(i64),
    /// Operator taking an operator kind.
    Operator(Operator),
    /// End-of-calculation.
    Eoc,
    /// Invalid token taking the value parsed as invalid.
    Invalid(String),
}
...
/// # Arguments
///
/// - `source` - A buffered input source from which to read.
///
/// # Return value
///
...
///
/// ```
/// # use crate::rpn_calculator::*;
/// #
/// let mut s1 = "123".as_bytes();
/// assert_eq!(read_token(&mut s1).unwrap(), Token::Operand(123));
///
/// let mut s2 = "/".as_bytes();
/// assert_eq!(read_token(&mut s2).unwrap(), Token::Operator(Operator::Div));
///
/// let mut s3 = "ŮñĭçøƋɇ".as_bytes();
/// assert!(read_token(&mut s3).is_err());
/// ```
///
pub fn read_token(source: &mut impl BufRead) -> Result<Token> {
    let mut input = vec![];
...
/// # use crate::rpn_calculator::*;
/// #
/// assert_eq!(calculate(3, 4, Operator::Mul).unwrap(), 12);
/// assert_eq!(calculate(76, 4, Operator::Add).unwrap(), 80);
///
/// assert!(calculate(i128::MAX, 2, Operator::Mul).is_err()); // Overflow.
/// assert!(calculate(1, 0, Operator::Mod).is_err());         // Division by zero.
/// ```
///
pub fn calculate(lhs: i128, rhs: i128, o: Operator) -> Result<i128> {
    match o {
        Operator::Add => lhs
            .checked_add(rhs)
